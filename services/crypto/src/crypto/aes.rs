...
use ring::aead::{
    self, AES_128_GCM, AES_256_GCM, Aad, BoundKey, LessSafeKey, Nonce, SealingKey, UnboundKey,
};
...
pub fn decrypt_aes128(
    ...
    LessSafeKey::new(unbound_key).open_in_place(nonce, aad, &mut in_out)?;
    Ok(in_out)
}

pub fn encrypt_aes256(key: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, CryptoError> {}

pub fn decrypt_aes256(key: &[u8], ciphertext: &[u8]) -> Result<Vec<u8>, CryptoError> {}
